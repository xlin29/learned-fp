...
"""
evaluate.py — Main evaluation entry point. Three modes:
      vote          — single-visit identification (closed-set, §5.4 per-key setting / Figure 6)
      vote_temporal — temporal return tracking (paper §5.4, Table 2)
    Reads both bayes (--model-type bayes) and fixed (--model-type fixed) artifacts.

    Paper:    vote = within-DB_T0 closed set (§5.4 per-key setting / Figure 6) · vote_temporal = §5.4 Table 2 temporal
"""
...
def build_chronological_split_all_vis(device_key_lists, keys, max_query_pool=20):
    ...
    q = min(max_query_pool, ceil(0.2 * n)).
    Assumes device_key_lists[dev][key] is chronologically sorted (as produced by
    list_key_rgba_sorted_all_visits, which sorts by mtime then filename)."""
    import math
...
def mode_vote(args):
    ...
    (results_dir / "fp_vote_per_key.json").write_text(
...
def mode_vote_temporal(args):
    ...
    (results_dir / "temporal_devices_kept.json").write_text(
    ...
    (results_dir / "fp_temporal_eligible_by_k.json").write_text(
    ...
    summary = {
        ...
        "note": "enrollment=ALL T0 cal; query=k from T+130d; per-k filtering; PID matching; cosine prototype",
    }
    (results_dir / "vote_temporal_summary.json").write_text(_dumps(summary, indent=2), encoding="utf-8")
    print(_dumps(summary, indent=2))
...
def main():
    ...
    {"vote": mode_vote, "vote_temporal": mode_vote_temporal}[args.cmd](args)
